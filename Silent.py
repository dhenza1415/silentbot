...
SK = SK.LINE()
SK.login(token="En45TVT2cueFZFQ03s28.jbCYKqV9J5TlIAYrqu8bIa.hgJVpGzPeESl5kMsfMlQR3FWqgMT2DwGstZjasfjEQU=")
SK.loginResult()

S1 = SILENT.LINE()
S1.login(token="EnKCkKvDsEh0F6pW5bad.Wvu+dMkG395XOOdtVRCjdq.uPCGCj+VZnr2qUB5sGOG9Vm9Cot75G0qZ8g5pJ2KQJI=")
S1.loginResult()

S2 = SILENT.LINE()
S2.login(token="En1oTTUdLDncvCU64Zf3.4xqN84T1S5cNUGFR9jI9mW.CLSvaG0zB4S0DhvOsoX9YmYWjslT+irMJ5CQBcTz99g=")
S2.loginResult()

S3 = SILENT.LINE()
S3.login(token="token lo")
S3.loginResult()

S4 = SILENT.LINE()
S4.login(token="token lo")
S4.loginResult()

S5 = SILENT.LINE()
S5.login(token="token lo")
S5.loginResult()

print "TEAM BOT PROTECT"
reload(sys)
sys.setdefaultencoding('utf-8')
helpMessage= """SILENT KILLER
『TEAM BOT PROTECT̈́』
_____________
🔥 STYLE OF BOTS 🔥
🏁Bot1 rename:[text]
...
SILENT KILLER
 ____B_O_G____
"""
KAC=[SK,Amid,Bmid,Cmid,Dmid,Emid]
mid = SK.getProfile().mid
...
contact = SK.getProfile()
backup = SK.getProfile()
backup.displayName = contact.displayName
backup.statusMessage = contact.statusMessage
backup.pictureStatus = contact.pictureStatus

contact = S1.getProfile()
backup = S1.getProfile()
backup.displayName = contact.displayName
backup.statusMessage = contact.statusMessage
backup.pictureStatus = contact.pictureStatus

contact = S2.getProfile()
backup = S2.getProfile()
backup.displayName = contact.displayName
backup.statusMessage = contact.statusMessage
backup.pictureStatus = contact.pictureStatus

contact = S3.getProfile()
backup = S3.getProfile()
backup.displayName = contact.displayName
backup.statusMessage = contact.statusMessage
backup.pictureStatus = contact.pictureStatus

contact = S4.getProfile()
backup = S4.getProfile()
backup.displayName = contact.displayName
backup.statusMessage = contact.statusMessage
backup.pictureStatus = contact.pictureStatus

contact = S5.getProfile()
backup = S5.getProfile()
backup.displayName = contact.displayName
backup.statusMessage = contact.statusMessage
backup.pictureStatus = contact.pictureStatus
...
def NOTIFIED_READ_MESSAGE(op):
    ...
    try:
        if op.param1 in wait2['readPoint']:
            Name = SK.getContact(op.param2).displayName
    ...
    try:
        ...
        if op.type == 5:
            if wait["autoAdd"] == True:
                SK.findAndAddContactsByMid(op.param1)
                if (wait["message"] in [""," ","\n",None]):
                    pass
                else:
                    SK.sendText(op.param1,str(wait["message"]))
                        ...
                        try:
                            ...
                            try:
                                ...
                                try:
                                    ...
                                    try:
                                        G = S4.getGroup(op.param1)
				    except:
					try:
                                            G = S5.getGroup(op.param1)
                                        except:
                                            pass
        ...
        if op.type == 13:
                if op.param3 in mid:
                    if op.param2 in Amid:
                        ...
                        SK.acceptGroupInvitationByTicket(op.param1,Ticket)
                ...
                if op.param3 in Cmid:
                    if op.param2 in Dmid:
                        ...
                        X.preventJoinByTicket = False
                        ks.updateGroup(X)
                        Ti = S4.reissueGroupTicket(op.param1)
                ...
                if op.param3 in Dmid:
                    if op.param2 in Emid:
                        X = S5.getGroup(op.param1)
                        X.preventJoinByTicket = False
                        S5.updateGroup(X)
                        Ti = S5.reissueGroupTicket(op.param1)
                ...
                if op.param3 in Emid:
                    if op.param2 in mid:
                        X = SK.getGroup(op.param1)
                        X.preventJoinByTicket = False
                        SK.updateGroup(X)
                        Ti = SK.reissueGroupTicket(op.param1)
                        SK.acceptGroupInvitationByTicket(op.param1,Ticket)
                        X.preventJoinByTicket = True
                        SK.updateGroup(X)
                        Ti = SK.reissueGroupTicket(op.param1)
...
#======================================================
                if op.param3 in Bmid:
                    if op.param2 in mid:
                        ...
                        G.preventJoinByTicket = False
                        SK.updateGroup(G)
                        Ticket = SK.reissueGroupTicket(op.param1)
                        S3.acceptGroupInvitationByTicket(op.param1,Ticket)
                        G.preventJoinByTicket = True
                        SK.updateGroup(G)
                        Ticket = SK.reissueGroupTicket(op.param1)
                ...
                if op.param3 in Bmid:
                    if op.param2 in Dmid:
                        ...
                        G.preventJoinByTicket = False
                        S2.updateGroup(G)
                        Ticket = S2.reissueGroupTicket(op.param1)
                        S1.acceptGroupInvitationByTicket(op.param1,Ticket)
                        G.preventJoinByTicket = True
                        S2.updateGroup(G)
                        Ticket = S2.reissueGroupTicket(op.param1)
...
#===========================================
        ...
        if op.type == 13:
            if mid in op.param3:
                ...
                if wait["autoJoin"] == True:
                    if wait["autoCancel"]["on"] == True:
                        if len(G.members) <= wait["autoCancel"]["members"]:
                            SK.rejectGroupInvitation(op.param1)
                        else:
                            SK.acceptGroupInvitation(op.param1)
                    else:
                        SK.acceptGroupInvitation(op.param1)
                ...
                if matched_list == []:
                    ...
                    SK.cancelGroupInvitation(op.param1, matched_list)
            if Amid in op.param3:
                G = SK.getGroup(op.param1)
                if wait["autoJoin"] == True:
                    if wait["autoCancel"]["on"] == True:
                        if len(G.members) <= wait["autoCancel"]["members"]:
                            S1.rejectGroupInvitation(op.param1)
                        else:
                ...
                if matched_list == []:
                    ...
                    S1.cancelGroupInvitation(op.param1, matched_list)
            if Bmid in op.param3:
                G = SK.getGroup(op.param1)
                if wait["autoJoin"] == True:
                    if wait["autoCancel"]["on"] == True:
                        if len(G.members) <= wait["autoCancel"]["members"]:
                            S2.rejectGroupInvitation(op.param1)
                        else:
                ...
                if matched_list == []:
                    ...
                    S2.cancelGroupInvitation(op.param1, matched_list)
            if Cmid in op.param3:
                G = PB.getGroup(op.param1)
                if wait["autoJoin"] == True:
                    if wait["autoCancel"]["on"] == True:
                        if len(G.members) <= wait["autoCancel"]["members"]:
                            S3.rejectGroupInvitation(op.param1)
                        else:
                ...
                if matched_list == []:
                    ...
                    S4.cancelGroupInvitation(op.param1, matched_list)

        if op.type == 17:
            if op.param3 in wait["blacklist"]:
                if not op.param2 in Bots and admin:
                    random.choice(KAC).kickoutFromGroup(op.param1,[op.param3])
                    SK.sendText(op.param1,"blacklist users are not allowed to sign in  -_-")
                    c = Message(to=op.param1, from_=None, text=None, contentType=13)
	   ...
	   if wait["welcomemsg"] == True:
              if op.param2 not in Bots:
                 ginfo = SK.getGroup(op.param1)
                 SK.sendText(op.param1,PB.getContact(op.param2).displayName + wait["welmsg"]+ str(ginfo.name))
        if op.type == 11:
            if not op.param2 in Bots:
              if wait["qr"] == True:
                try:
                    klist=[S1,S2,S3,S4,S5]
                    kicker = random.choice(klist)
                    G = kicker.getGroup(op.param1)
                    G.preventJoinByTicket = True
            ...
            if not op.param2 in Bots and admin:
              if wait["protectionOn"] == True:
                 try:
                    ...
                    G.preventJoinByTicket = True
                    kicker.updateGroup(G)
                    SK.sendText(op.param1,"please do not open link group-_-")
                    c = Message(to=op.param1, from_=None, text=None, contentType=13)
                    c.contentMetadata={'mid':op.param2}
                    SK.sendMessage(c)
            ...
            if not op.param2 in Bots and admin:
                if wait["protectionOn"] == True:
                    ...
                    if G is not None:
                        ...
                        kicker.kickoutFromGroup(op.param1,[op.param2])
                        SK.sendText(op.param1,"you are prohibited from inviting-_-")
                        c = Message(to=op.param1, from_=None, text=None, contentType=13)
                        c.contentMetadata={'mid':op.param2}
                        SK.sendMessage(c)
        if op.type == 15:
        ...
        if op.type == 19:
                if not op.param2 in Bots:
                    try:
                        gs = S1.getGroup(op.param1)
                        gs = S2.getGroup(op.param1)
                        targets = [op.param2]
        ...
        if op.type == 19:
                if mid in op.param3:
                    ...
                    ki.updateGroup(G)
                    Ti = S1.reissueGroupTicket(op.param1)
                    SK.acceptGroupInvitationByTicket(op.param1,Ti)
                    ...
                    S4.acceptGroupInvitationByTicket(op.param1,Ti)
                    S5.acceptGroupInvitationByTicket(op.param1,Ti)
                    X = SK.getGroup(op.param1)
                    X.preventJoinByTicket = True
                    SK.updateGroup(X)
                    Ti = SK.reissueGroupTicket(op.param1)
                ...
                if Cmid in op.param3:
                    ...
                    X.preventJoinByTicket = True
                    S3.updateGroup(X)
                    Ticket = S3.reissueGroupTicket(op.param1)
                    if op.param2 in wait["blacklist"]:
                        pass
                    if op.param2 in wait["whitelist"]:
                        pass
                ...
                if Dmid in op.param3:
                    ...
                    X.preventJoinByTicket = True
                    S4.updateGroup(X)
                    Ticket = S4.reissueGroupTicket(op.param1)
                    if op.param2 in wait["blacklist"]:
                        pass
                    if op.param2 in wait["whitelist"]:
                        pass
...
#========================================================================
                if Fmid in op.param3:
                    ...
                    S4.acceptGroupInvitationByTicket(op.param1,Ti)
                    S5.acceptGroupInvitationByTicket(op.param1,Ti)
                    S6.acceptGroupInvitationByTicket(op.param1,Ti)
                    S7.acceptGroupInvitationByTicket(op.param1,Ti)
                    S8.acceptGroupInvitationByTicket(op.param1,Ti)
                    S9.acceptGroupInvitationByTicket(op.param1,Ti)
                    S10.acceptGroupInvitationByTicket(op.param1,Ti)
                    S11.acceptGroupInvitationByTicket(op.param1,Ti)
                    S12.acceptGroupInvitationByTicket(op.param1,Ti)
                    S13.acceptGroupInvitationByTicket(op.param1,Ti)
                    S14.acceptGroupInvitationByTicket(op.param1,Ti)
                    S15.acceptGroupInvitationByTicket(op.param1,Ti)
                    X = S1.getGroup(op.param1)
                    X.preventJoinByTicket = True
                    S1.updateGroup(X)
                    Ticket = S1.reissueGroupTicket(op.param1)
                ...
                if Gmid in op.param3:
                    ...
                    S14.acceptGroupInvitationByTicket(op.param1,Ti)
                    S15.acceptGroupInvitationByTicket(op.param1,Ti)
                    S16.acceptGroupInvitationByTicket(op.param1,Ti)
                    X = S6.getGroup(op.param1)
                    X.preventJoinByTicket = True
                    kg.updateGroup(X)
...
#==================================================
              ...
              if msg.from_ in admin:
                try:
                    ...
                    details = "\n======INSTAGRAM INFO USER======"
                    SK.sendText(msg.to, detail + user + user1 + followers + following + post + link + details)
                    SK.sendImageWithURL(msg.to, text1[0])
                except Exception as njer:
                	SK.sendText(msg.to, str(njer))
              ...
              if msg.from_ in admin:
                try:
                    ...
                    for song in data:
                        ...
                        SK.sendText(msg.to, hasil)
                        SK.sendText(msg.to, "serching")
                        SK.sendAudioWithURL(msg.to, song[3])
		except Exception as njer:
